...
while(quit == False):
	...
	while run:
		...
		if (
			my_snake.nodes[len(my_snake.nodes)-1].x >= 500 or
			my_snake.nodes[len(my_snake.nodes)-1].y >= 500
		): #boundaries
			run = False
			break
		if (
			my_snake.nodes[len(my_snake.nodes)-1].x <= 0 or 
			my_snake.nodes[len(my_snake.nodes)-1].y <= 0
		):
			run = False
			break
		...
		for node in my_snake.nodes: #draw the snake
			...
			pygame.draw.rect(
				win,(0,255,0),(headx,heady,my_snake.width,my_snake.width)
				)
			pygame.draw.rect(
				win,(255,0,0),(apple.x,apple.y,apple.width,apple.width)
				) #draw the apple
